...
#define CSM_KEYFILE_SESSION_GROUP "Sagarmatha Session"
...
#define CSM_KEYFILE_REQUIRED_COMPONENTS_KEY "RequiredComponents"
#define CSM_KEYFILE_REQUIRED_PROVIDERS_KEY  "RequiredProviders"
#define CSM_KEYFILE_DEFAULT_PROVIDER_PREFIX "DefaultProvider"

/* See https://bugzilla.gnome.org/show_bug.cgi?id=641992 for discussion */
#define CSM_RUNNABLE_HELPER_TIMEOUT 3000 /* ms */

typedef void (*CsmFillHandleProvider) (const char *provides,
                                       const char *default_provider,
                                       const char *app_path,
                                       gpointer    user_data);
typedef void (*CsmFillHandleComponent) (const char *component,
                                        const char *app_path,
                                        gpointer    user_data);
...
handle_default_providers (GKeyFile              *keyfile,
                          ...
                          CsmFillHandleProvider  callback,
                          gpointer               user_data)
{
        ...
        default_providers = g_key_file_get_string_list (keyfile,
                                                        CSM_KEYFILE_SESSION_GROUP,
                                                        CSM_KEYFILE_REQUIRED_PROVIDERS_KEY,
                                                        NULL, NULL);
        ...
        for (i = 0; default_providers[i] != NULL; i++) {
                ...
                key = g_strdup_printf ("%s-%s",
                                       CSM_KEYFILE_DEFAULT_PROVIDER_PREFIX,
                                       default_providers[i]);
                value = g_key_file_get_string (keyfile,
                                               CSM_KEYFILE_SESSION_GROUP, key,
                                               NULL);
                g_free (key);
                ...
                app_path = csm_util_find_desktop_file_for_app_name (value,
...
handle_required_components (GKeyFile               *keyfile,
                            gboolean                look_in_saved_session,
                            CsmFillHandleComponent  callback,
                            gpointer                user_data)
{
        ...
        required_components = g_key_file_get_string_list (keyfile,
                                                          CSM_KEYFILE_SESSION_GROUP,
                                                          CSM_KEYFILE_REQUIRED_COMPONENTS_KEY,
                                                          NULL, NULL);
        ...
        for (i = 0; required_components[i] != NULL; i++) {
                ...
                app_path = csm_util_find_desktop_file_for_app_name (required_components[i],
                                                                    look_in_saved_session, TRUE);
...
static void
maybe_load_saved_session_apps (CsmManager *manager)
{
        CsmSystem *system;
        gboolean is_login;
...
append_required_providers_helper (const char *provides,
                                  ...
                                  gpointer    user_data)
{
        CsmManager *manager = user_data;

        if (app_path == NULL)
                g_warning ("Unable to find default provider '%s' of required provider '%s'",
                           default_provider, provides);
        else
                csm_manager_add_required_app (manager, app_path, provides);
}
                                   ...
                                   gpointer    user_data)
{
        ...
        if (app_path == NULL)
                ...
                csm_manager_add_required_app (manager, app_path, NULL);
}
                    ...
                    GKeyFile   *keyfile)
{
        ...
        if (!csm_manager_get_failsafe (manager)) {
                char **autostart_dirs;
                int    i;
                ...
                for (i = 0; autostart_dirs[i]; i++) {
                        csm_manager_add_autostart_apps_from_dir (manager,
                                                                 autostart_dirs[i]);
                }
        ...
        handle_default_providers (keyfile, !csm_manager_get_failsafe (manager),
...
get_session_keyfile_if_valid (const char *path)
{
        ...
        if (!g_key_file_has_group (keyfile, CSM_KEYFILE_SESSION_GROUP)) {
                g_warning ("Cannot use session '%s': no '%s' group.", path, CSM_KEYFILE_SESSION_GROUP);
                goto error;
        }
        ...
        if (list != NULL) {
                ...
                for (i = 0; list[i] != NULL; i++) {
                        key = g_strdup_printf ("%s-%s", CSM_KEYFILE_DEFAULT_PROVIDER_PREFIX, list[i]);
                        value = g_key_file_get_string (keyfile,
                                                       CSM_KEYFILE_SESSION_GROUP, key,
                                                       NULL);
                        g_free (key);
...
find_valid_session_keyfile (const char *session)
{
        ...
        for (i = 0; i < dirs->len; i++) {
                path = g_build_filename (dirs->pdata[i], "sagarmatha-session", "sessions", basename, NULL);
                     ...
                     gboolean   *is_fallback)
{
        ...
        *actual_session = NULL;
        ...
        value = g_key_file_get_string (keyfile,
                                       CSM_KEYFILE_SESSION_GROUP, CSM_KEYFILE_RUNNABLE_KEY,
                                       NULL);
        if (!IS_STRING_EMPTY (value)) {
                g_debug ("fill: *** Launching helper '%s' to know if session is runnable", value);
                session_runnable = csm_process_helper (value, CSM_RUNNABLE_HELPER_TIMEOUT, &error);
                if (!session_runnable) {
        ...
        value = g_key_file_get_string (keyfile,
                                       CSM_KEYFILE_SESSION_GROUP, CSM_KEYFILE_FALLBACK_KEY,
                                       NULL);
...
set_xdg_current_desktop (GKeyFile *keyfile)
{
        ...
        value = g_key_file_get_string (keyfile,
                                       CSM_KEYFILE_SESSION_GROUP, CSM_KEYFILE_DESKTOP_NAME_KEY,
                                       NULL);

        if (!IS_STRING_EMPTY (value)) {
                csm_util_setenv ("XDG_CURRENT_DESKTOP", value);
        }
        else {
                csm_util_setenv ("XDG_CURRENT_DESKTOP", "GNOME");
        }
        g_free (value);
}

gboolean
csm_session_fill (CsmManager  *manager,
                  const char  *session)
{
        GKeyFile *keyfile;
        gboolean is_fallback;
