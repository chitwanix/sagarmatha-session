...
/* Tool to set the property _SAGARMATHA_SESSION_ACCELERATED on the root window */
/*
...
/* Values used for the _SAGARMATHA_SESSION_ACCELERATED root window property */
#define NO_ACCEL            0
#define HAVE_ACCEL          1
...
main (int argc, char **argv)
{
        GdkDisplay *display = NULL;
        int estatus;
        char *child_argv[] = { LIBEXECDIR "/sagarmatha-session-check-accelerated-helper", NULL };
        Window rootwin;
        ...
        is_accelerated_atom = gdk_x11_get_xatom_by_name_for_display (display, "_SAGARMATHA_SESSION_ACCELERATED");
 ...
 read:
        ...
        if (!g_spawn_sync (NULL, (char**)child_argv, NULL, 0,
                ...
                g_printerr ("sagarmatha-session-check-accelerated: Failed to run helper: %s\n", error->message);
                g_clear_error (&error);
        } else {
                is_accelerated = (estatus == 0);
                if (!is_accelerated)
                        g_printerr ("sagarmatha-session-check-accelerated: Helper exited with code %d\n", estatus);
        }
