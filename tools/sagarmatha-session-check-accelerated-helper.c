/* gcc -o sagarmatha-session-accelerated `pkg-config --cflags --libs xcomposite gl` -Wall sagarmatha-session-is-accelerated.c */
 ...
 *    sagarmatha-session to directly try and do any GL detection; better to
 *    use a helper binary.
 *
...
{
        fprintf (stderr, "sagarmatha-session-is-accelerated: %s\n", str);
}
